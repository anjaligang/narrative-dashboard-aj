...
d3.csv('Barbie_State_Search.csv').then(data => {
  ...
  // // Helper function to set the color based on search data value
  // function getColor(searchValue) {
  //   // Using a color scale with shades of pink
  //   const colorScale = d3.scaleSequential().domain([100, 0]).interpolator(d3.interpolateRdPu);
  //   return colorScale(searchValue);
  // }

  function getColor(searchValue) {
    // Using a color scale with shades of blue from light to dark (reversed domain)
    const colorScale = d3.scaleSequential().domain([0, 100]).interpolator(d3.interpolateRdPu);
    return colorScale(searchValue);
  }
